...
Remote mode requires a dedicated HTTPS management origin and Secure host-only cookies. The root-policy-managed admin vhost requires mTLS, CRL and application password authentication. New installs do not bind administrators to a fixed egress IP; `--admin-cidr` optionally adds source-IP restrictions. Existing installations retain their previous policy on upgrade. Business hosts must differ from the management host. Remote business routes require TLS and a scoped API key or mTLS; shared admin sessions, legacy E5 auth and public routes are rejected.
...
Use `bash deploy/full-deploy.sh --dry-run` to inspect the plan. Installation requires an actual domain, email and explicit `--agree-tos`; a fixed admin source IP is not required. The browser p12 and AES-GCM-encrypted CA recovery archive require a user-held passphrase. Let's Encrypt renews automatically. A separate daily systemd task refreshes the management CRL when 30 days or less remain, without rotating browser certificates. Existing installs enable it once with `pki-auto-enable`; a root-only, host-encrypted systemd credential unlocks the authoritative recovery archive. This grants the host online signing capability and is not an offline-CA guarantee. No plaintext CA signing key remains on disk after bootstrap. Certificate verification/renewal failures are not treated as successful deployment.

## Account and operations updates

All users can change their own password after verifying the current password; every session is then revoked. Administrators can download the encrypted existing browser P12 through an authenticated CSRF-protected POST with password re-verification, never a public link. The initial mTLS bootstrap remains required. No CA/probe key or archive is downloaded.
