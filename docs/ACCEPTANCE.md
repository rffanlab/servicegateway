...
- [ ] 每个实际服务通过新端口完成登录、静态资源、页面刷新、重定向、Cookie、API、上传下载测试。
...
验收前保留新旧入口并行。逐个服务切换客户端地址，每次保留旧入口回退。不要一次切全部服务，也不要因为网关发布成功就删除旧配置。完成现场验收后，再审核合并 `feat/e5-mysql-gateway` 到 `main`。
