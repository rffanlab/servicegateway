...
> 本文替代旧版“局域网即可访问”的部署默认值。默认模式为 `remote`。这不是公网安全认证或零漏洞保证；完成自动化测试之后，仍必须在目标主机验收网络、身份、业务隔离和恢复。

## 1. 默认拒绝暴露

管理入口仅监听 `127.0.0.1:19091`，管理 API 仅监听 `127.0.0.1:19092`，数据面状态端口仅监听 `127.0.0.1:19093`；Agent 只提供有 Unix 用户校验的本机 socket。默认 edge 也只监听 `127.0.0.1`，IP 白名单仅允许 loopback。不开放 MySQL、Docker socket、模型服务、ComfyUI 或 Harness 内部端口。
...
| 远程路由 | 必须精确域名、TLS、`api_key` 或 `mtls`，必须设置正数限流；不允许使用管理域名 |
...
2. 克隆并审核 feature 分支。先运行 `sudo bash deploy/install.sh` 生成 root-only 环境模板；配置独立 MySQL 数据库 `servicegateway` 与专用用户。
...
6. 配置管理专用域名的 HTTPS 入口，优先放在 VPN/受限网络。需要直接外部访问时，用 `deploy/nginx-console-tls.conf.example`：受信服务器证书 + 客户端证书校验 + 客户端 CA 撤销列表 + 管理来源 IP 白名单，再保留应用密码登录。
7. 模板中的 `admin.example.invalid` 和 `192.0.2.10/32` 是占位符，不是实际配置。证书、CA、CRL、权限、域名和安全组尚未配置时，不应开放 443。不能把内部 :19091 或 :19092 直接做公网端口映射。
