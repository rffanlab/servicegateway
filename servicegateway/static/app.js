...
async function load(){
  ...
  if(view==='overview'){
    ...
    html=`<div class="section-head"><h2>自动化与网关访问凭据</h2>${button('创建密钥','key-new','','primary')}</div><p class="hint">使用 X-Gateway-Key 请求头。路由作用域只能访问指定路由，注册作用域只能登记指定服务，不能控制启停。密钥明文只返回一次。</p>`+(rows.length?table(['名称','路由作用域','登记作用域','业务用户查询','到期 / 状态','操作'],rows.map(k=>[esc(k.name),esc(k.route_ids.join(', ')),esc(k.service_ids.join(', ')),esc((k.user_service_ids??[]).join(', ')),`${esc(new Date(k.expires_at).toLocaleDateString())} ${badge(k.revoked?'已撤销':'有效',k.revoked?'':'good')}`,k.revoked?'—':button('撤销','key-revoke',k.id,'danger')])):empty('暂无 API Key','不创建全局万能密钥，按路由或注册任务分配权限。'));
  }else if(view==='users'){
...
async function routeEditor(id, copy=false){
  ...
  edit(copy?'复制路由草稿':(id?'编辑路由草稿':'新增路由草稿'),fields,async f=>{const data={...r}; ['id','name','service_id','host','path','auth','balance'].forEach(k=>data[k]=f.get(k)); ['listen_port','timeout_seconds','max_body_mb','rate_per_second','burst','max_connections'].forEach(k=>data[k]=Number(f.get(k))); ['enabled','websocket','buffering','strip_prefix'].forEach(k=>data[k]=f.has(k)); data.upstreams=JSON.parse(f.get('upstreams'));data.certificate=f.get('certificate')||null;data.client_ca=f.get('client_ca')||null;data.upstream_auth={mode:f.get('upstream_auth')};data.session_users=f.get('session_users').split(',').map(x=>x.trim()).filter(Boolean);data.business_roles=f.get('business_roles').split(',').map(x=>x.trim()).filter(Boolean);data.allow_cidrs=f.get('allow_cidrs').split(',').map(x=>x.trim()).filter(Boolean);if(old&&data.id!==old.id)throw new Error('编辑不能更改路由 ID');if(!overview.assets.some(s=>s.id===data.service_id))throw new Error('请选择已登记服务');await api(`/api/routes/${encodeURIComponent(data.id)}?revision=${overview.revision}`,'PUT',data);});
}
