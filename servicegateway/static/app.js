...
import {cloneRouteDraft} from './route-tools.js';
import {businessUserUI} from './business-user-ui.js';
const $ = s => document.querySelector(s);
const esc = v => String(v ?? '').replace(/[&<>"']/g, x => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[x]));
let me = null, view = 'overview', overview = null, editorSave = null, toastTimer;
const titles = {databases:'业务数据库','business-users':'业务用户',overview:'运行总览',services:'服务管理',routes:'网关路由',releases:'发布与回滚',traffic:'访问采样',keys:'API 密钥',users:'用户与权限',audit:'操作审计',account:'账户与证书'};
...
async function load(){
  ...
  if(view==='business-users') businessUsers.bind(()=>load().catch(e=>toast(e.message)));
}
...
async function routeEditor(id, copy=false){
  ...
  const fields=field('id','路由 ID',r.id)+field('name','显示名称',r.name)+field('service_id','所属服务',r.service_id,'select',overview.assets.map(s=>[s.id,s.name]))+(remote?'<label>统一 HTTPS 入口<input name="listen_port" type="number" value="443" readonly></label>':field('listen_port','LAN 监听端口',r.listen_port,'number'))+field('host',remote?'业务域名（与管理域名分离）':'精确域名；_ 为 LAN 默认入口',r.host)+field('path','路径前缀；必须以 / 结尾',r.path)+field('auth','边缘鉴权',authValue,'select',[...(remote?[]:[['session','ServiceGateway 登录'],['e5','旧 E5 统一登录'],['public','无需登录（仅 LAN）']]),['api_key','仅 API Key'],['mtls','仅客户端证书'],['mtls_or_api_key','客户端证书 或 API Key（任一通过即可）'],['wechat','微信登录 Token']])+field('client_ca','客户端 CA 目录 ID（证书鉴权必填）',r.client_ca??'')+field('upstream_auth','上游身份确认',r.upstream_auth?.mode??'none','select',[['none','不注入路由 Secret'],['route_secret','注入每路由独立 Secret']])+`<p class="hint full">所属服务必须从已登记服务中选择，不能手填。当前所选服务的来源 CIDR 上限：${esc(sourceScope)}。路由留空时继承服务上限；微信登录路由会强制使用每路由 Secret，上游 Secret 不显示在页面中。</p>`+field('session_users','允许会话用户（逗号分隔；空则仅管理员）',(r.session_users??[]).join(','))+field('user_roles','微信用户角色（逗号分隔；空=任意已登录用户）',(r.user_roles??[]).join(','))+field('max_connections','每 IP 最大同时连接数',r.max_connections??16,'number')+field('balance','负载均衡',r.balance,'select',[['round_robin','加权轮询'],['least_conn','最少连接'],['ip_hash','来源 IP 绑定']])+field('upstreams','上游列表 JSON',JSON.stringify(r.upstreams,null,2),'textarea')+field('timeout_seconds','读取/发送空闲超时（秒）',r.timeout_seconds,'number')+field('max_body_mb','上传上限（MB）',r.max_body_mb,'number')+field('rate_per_second','每来源 IP 限流（0 不限制）',r.rate_per_second,'number')+field('burst','突发额度',r.burst,'number')+field('certificate',remote?'证书目录 ID（必填）':'证书目录 ID（可空）',r.certificate??'')+field('allow_cidrs','来源 IP 白名单（逗号分隔；空则继承所属服务上限）',r.allow_cidrs.join(','))+field('enabled','启用此路由',r.enabled,'checkbox')+field('websocket','WebSocket 转发',r.websocket,'checkbox')+field('buffering','启用响应缓冲（SSE 应关闭）',r.buffering,'checkbox')+field('strip_prefix','移除路径前缀（业务程序需支持）',r.strip_prefix,'checkbox');
  edit(copy?'复制路由草稿':(id?'编辑路由草稿':'新增路由草稿'),fields,async f=>{const data={...r}; ['id','name','service_id','host','path','auth','balance'].forEach(k=>data[k]=f.get(k)); ['listen_port','timeout_seconds','max_body_mb','rate_per_second','burst','max_connections'].forEach(k=>data[k]=Number(f.get(k))); ['enabled','websocket','buffering','strip_prefix'].forEach(k=>data[k]=f.has(k)); data.upstreams=JSON.parse(f.get('upstreams'));data.certificate=f.get('certificate')||null;data.client_ca=['mtls','mtls_or_api_key'].includes(data.auth)?(f.get('client_ca')||null):null;data.upstream_auth={mode:data.auth==='wechat'?'route_secret':f.get('upstream_auth')};data.session_users=f.get('session_users').split(',').map(x=>x.trim()).filter(Boolean);data.user_roles=data.auth==='wechat'?f.get('user_roles').split(',').map(x=>x.trim()).filter(Boolean):[];data.allow_cidrs=f.get('allow_cidrs').split(',').map(x=>x.trim()).filter(Boolean);if(old&&data.id!==old.id)throw new Error('编辑不能更改路由 ID');if(!overview.assets.some(s=>s.id===data.service_id))throw new Error('请选择已登记服务');await api(`/api/routes/${encodeURIComponent(data.id)}?revision=${overview.revision}`,'PUT',data);});
}
async function perform(action,id){
  if(await databases.perform(action,id))return;
  if(await businessUsers.perform(action,id)){await load();return;}
