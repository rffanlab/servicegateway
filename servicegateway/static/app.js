...
async function load(){
  ...
  if(view==='overview'){
    ...
    window.sgKeys=rows;
    html=`<div class="section-head"><h2>自动化与网关访问凭据</h2>${button('创建密钥','key-new','','primary')}</div><p class="hint">使用 X-Gateway-Key 请求头。管理员可随时查看并复制完整 Key；数据库保存的是加密副本，鉴权仍使用不可逆哈希。旧版只保存哈希的 Key 无法恢复，需要重新创建一次。</p>`+(rows.length?table(['名称','密钥','路由作用域','登记作用域','业务用户查询','到期 / 状态','操作'],rows.map(k=>[
      esc(k.name),
      k.token_recoverable?`<code class="secret-code">${esc(k.token)}</code><div class="row-note">${button('复制','key-copy',k.id)}</div>`:'<span class="muted">旧密钥不可恢复，请重建</span>',
      esc(k.route_ids.join(', ')),esc(k.service_ids.join(', ')),esc((k.user_service_ids??[]).join(', ')),
      `${k.never_expires?'永不过期':esc(new Date(k.expires_at).toLocaleDateString())} ${badge(k.revoked?'已撤销':'有效',k.revoked?'':'good')}`,
      k.revoked?'—':button('撤销','key-revoke',k.id,'danger')
    ])):empty('暂无 API Key','按实际路由或服务作用域创建密钥。'));
  }else if(view==='users'){
...
仅访问 127.0.0.1:19092/internal/registry/services；服务名必须在 Key 作用域和本机授权中。没有匿名本机免登录接口。`);return;
  ...
  if(action==='preview'){const p=await api('/api/gateway/preview','POST');const missing=p.missing_upstream_secrets??[];if(missing.length){const commands=missing.map(routeId=>`sudo /srv/e5-apps/servicegateway/current/.venv/bin/sgctl route-secret --route ${routeId} --output /root/${routeId}-gateway.env`).join('\n');read('发布已阻止：缺少上游路由 Secret',`以下路由启用了“注入每路由独立 Secret”，但尚未配置 Secret：\n\n${missing.join(', ')}\n\n如果这些路由并不需要上游 Secret，请返回路由编辑，把“上游身份确认”改成“不注入路由 Secret”。\n\n如果业务服务确实会校验 X-SG-Upstream-Token，请分别执行：\n\n${commands}\n\n把生成的 Secret 安全配置给对应业务服务后，再重新预览发布。`);return;}edit('发布预览',`<p class="hint full">草稿版本 ${p.revision} · ${esc(p.digest)}<br>将执行二次校验、nginx -t、reload 与配置指纹检查。失败时恢复旧配置。</p><pre class="readbox">${esc(p.config)}</pre>`+field('note','发布说明','','text'),async f=>{await api('/api/gateway/publish','POST',{revision:p.revision,digest:p.digest,note:f.get('note')});},'确认发布');return;}
  ...
  if(action==='key-copy'){
    const key=(window.sgKeys??[]).find(k=>k.id===id);
    if(!key?.token)throw new Error('这个旧密钥没有可恢复的明文，请重新创建');
    try{await navigator.clipboard.writeText(key.token);toast('密钥已复制到剪贴板');}
    catch(_){read('复制密钥',key.token+'\n\n浏览器未允许自动写入剪贴板，请手动复制上面的完整 Key。');}
    return;
  }
