...
async function routeEditor(id){
  ...
  const fields=field('id','路由 ID',r.id)+field('name','显示名称',r.name)+field('service_id','所属服务',r.service_id,'select',overview.assets.map(s=>[s.id,s.name]))+(remote?'<label>统一 HTTPS 入口<input name="listen_port" type="number" value="443" readonly></label>':field('listen_port','LAN 监听端口',r.listen_port,'number'))+field('host',remote?'业务域名（与管理域名分离）':'精确域名；_ 为 LAN 默认入口',r.host)+field('path','路径前缀；必须以 / 结尾',r.path)+field('auth','鉴权',r.auth,'select',[...(remote?[]:[['session','ServiceGateway 登录'],['e5','旧 E5 统一登录'],['public','无需登录（仅 LAN）']]),['api_key','限定 API Key'],['mtls','客户端证书（远程网页）']])+field('client_ca','客户端 CA 目录 ID（mTLS 必填）',r.client_ca??'')+field('session_users','允许会话用户（逗号分隔；空则仅管理员）',(r.session_users??[]).join(','))+field('max_connections','每 IP 最大同时连接数',r.max_connections??16,'number')+field('balance','负载均衡',r.balance,'select',[['round_robin','加权轮询'],['least_conn','最少连接'],['ip_hash','来源 IP 绑定']])+field('upstreams','上游列表 JSON',JSON.stringify(r.upstreams,null,2),'textarea')+field('timeout_seconds','读取/发送空闲超时（秒）',r.timeout_seconds,'number')+field('max_body_mb','上传上限（MB）',r.max_body_mb,'number')+field('rate_per_second','每来源 IP 限流（0 不限制）',r.rate_per_second,'number')+field('burst','突发额度',r.burst,'number')+field('certificate',remote?'证书目录 ID（必填）':'证书目录 ID（可空）',r.certificate??'')+field('allow_cidrs','IP 白名单（逗号分隔；空则使用 root 策略）',r.allow_cidrs.join(','))+field('enabled','启用此路由',r.enabled,'checkbox')+field('websocket','WebSocket 转发',r.websocket,'checkbox')+field('buffering','启用响应缓冲（SSE 应关闭）',r.buffering,'checkbox')+field('strip_prefix','移除路径前缀（业务程序需支持）',r.strip_prefix,'checkbox');
