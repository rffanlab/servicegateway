...
def validate_snapshot(snapshot, policy, inspect_units=True):
    ...
    for route in snapshot.routes:
        if policy.get("remote_mode", True):
            ...
            if route.auth not in ("api_key", "mtls", "mtls_or_api_key", "mtls_api_key", "wechat") or not route.certificate or route.host == "_":
                raise ValueError("Remote routes require exact host, TLS and API key, mTLS, certificate/API-key fallback, or WeChat login")
